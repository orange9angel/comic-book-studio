#!/usr/bin/env python3
"""
Comic Book Studio — ElevenLabs TTS 配音生成器

功能：
- 使用 ElevenLabs API 生成高质量中文 TTS
- 免费版可用声音：Sarah (EXAVITQu4vr4xnSDxMaL)
- API 失败时自动回退到 edge-tts
- 生成 TTS、翻页音效、BGM 并混音为最终音频

Usage:
    $env:ELEVENLABS_API_KEY="sk_xxx"
    python build_audio_elevenlabs.py
"""
import asyncio
import json
import os
...
def generate_with_elevenlabs(text, output_path, speed=1.0):
    """调用 ElevenLabs API。成功返回 True，失败返回 False。"""
    ...
    try:
        ...
        settings = ELEVENLABS_VOICE.copy()
        settings["speed"] = speed
        del settings["voice_id"]
...
def build_lines(story, config):
    ...
    for page_idx, page in enumerate(pages):
        ...
        narrator_dur = 0.0
        if caption:
            narrator_cfg = roles.get("旁白", {"voice": "zh-CN-YunxiNeural", "rate": "+0%", "pitch": "+0Hz", "volume": "+0%"})
        ...
        for i, bubble in enumerate(bubbles):
            ...
            speed_map = {
                "neutral": 1.0, "cheerful": 1.05, "excited": 1.1,
                "surprised": 1.08, "curious": 1.03, "friendly": 1.0,
                "sad": 0.92, "angry": 1.05, "terrified": 1.12,
            }
            el_speed = speed_map.get(emotion, 1.0)
...
async def generate_voice(line):
    """生成单条语音，优先 ElevenLabs，失败回退 edge-tts。"""
    out = AUDIO_DIR / line["file"]

    # 尝试 ElevenLabs
    if ELEVENLABS_API_KEY and line["type"] == "dialogue":
        print(f"  ElevenLabs: {line['speaker']} - {line['text'][:15]}...")
        if generate_with_elevenlabs(line["text"], out, line.get("elevenlabs_speed", 1.0)):
            print(f"    ✅ 成功")
            return
...
def mix_dialogue(lines):
    ...
    dialogue = AUDIO_DIR / "dialogue_elevenlabs.wav"
    run([
        "ffmpeg", "-y",
        *voice_inputs,
...
async def main():
    print("=" * 50)
    print("Comic Book Studio — ElevenLabs TTS 配音生成器")
    print("=" * 50)

    if not ELEVENLABS_API_KEY:
        print("[WARNING] ELEVENLABS_API_KEY 未设置，将使用 edge-tts")
    else:
        print(f"[INFO] ElevenLabs API Key 已设置: {ELEVENLABS_API_KEY[:12]}...")
    ...
    if ELEVENLABS_API_KEY:
        print(f"ElevenLabs 免费额度: 10,000/月")
    print(f"{'=' * 50}")
